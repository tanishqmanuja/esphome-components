...
uint16_t DDPAddressableLightEffect::process_(const uint8_t *payload, uint16_t size, uint16_t used) {
  ...
  switch (this->scaling_mode_) {
    ...
    case DDP_SCALE_PIXEL:  // pixel scaling occurs at the pixel level, no need to scan here but we still need brightness
                           // maxed.
      set_max_brightness_();
    default:
...
float DDPAddressableLightEffect::scan_packet_and_return_multiplier_(const uint8_t *payload, uint16_t start,
                                                                    uint16_t end) {
  uint8_t max_val = 0;
...
float DDPAddressableLightEffect::multiplier_from_max_val_(uint8_t max_val) {
  if (max_val == 0) {
    ...
    return this->state_->remote_values.get_brightness() * 255.0f / (float) max_val;
  }
}
